...
new_content = """use std::io::BufRead;
use std::str::FromStr;
...
fn parse_bias_record(line: &str) -> Option<BiasRecord> {
    if line.len() < 90 || line.starts_with('*') { return None; }
    ...
    let constell = match prn_str.chars().next()? {
        'G' => Constellation::Gps, 'R' => Constellation::Glonass, 'E' => Constellation::Galileo,
        'C' => Constellation::Beidou, 'J' => Constellation::Qzss, _ => return None,
    };
    let prn = prn_str[1..3].trim().parse::<u8>().ok()?;
    let sat = SatelliteId { constellation: constell, prn };
    
    let st_str = line[15..24].trim();
    let station = if st_str.is_empty() { None } else { Some(st_str.to_string()) };
    ...
    let std_dev = if line.len() >= 102 { line[92..103].trim().parse::<f64>().unwrap_or(0.0) } else { 0.0 };
...
impl SinexBias {
    pub fn parse<R: BufRead>(mut reader: R) -> Result<Self, String> {
        ...
        while let Ok(bytes_read) = reader.read_until(b'\\n', &mut buf) {
            ...
            if line.starts_with("+BIAS/SOLUTION") { in_solution = true; continue; }
            if line.starts_with("-BIAS/SOLUTION") { in_solution = false; continue; }
            
            if in_solution {
                if let Some(record) = parse_bias_record(&line) {
                    records.push(record);
                }
            }
        }
    ...
    pub fn get_bias(&self, sat: SatelliteId, obs: ObsCode, t: GpsTime) -> Option<f64> {
        if let Some(val) = self.get_exact_bias(sat, obs, t) { return Some(val); }
        
        let o_str = obs.to_string();
        let fb = match (sat.constellation, o_str.as_str()) {
            (Constellation::Gps, "C1C") => "C1W",
            (Constellation::Gps, "L1C") => "L1W",
            (Constellation::Gps, "C2X" | "C2L" | "C2S") => "C2W",
            (_, "C2X" | "C2L" | "C2S") => "C2C",
            (Constellation::Gps, "L2X" | "L2L" | "L2S") => "L2W",
            (_, "L2X" | "L2L" | "L2S") => "L2C",
            _ => return None,
        };
        
        if let Ok(code) = ObsCode::from_str(fb) {
...
"#;
        ...
        assert_eq!(r1.obs1.to_string(), "C1C");
        assert_eq!(r1.value, -2.5186);
        
        let r2 = &bias.records[1];
        assert_eq!(r2.bias_type, BiasType::Dcb);
        assert_eq!(r2.obs1.to_string(), "C1W");
        assert_eq!(r2.obs2.unwrap().to_string(), "C2W");
    }
}
