...
enum Commands {
    ...
    Process {
        ...
        #[arg(
            long,
            help = "Lever arm from IMU to GNSS antenna (x,y,z in meters)",
            default_value = "0,0,0"
        )]
        lever_arm: String,
        ...
        #[arg(
            long,
            help = "Automatically calibrate 6-DOF extrinsics and GNSS intrinsics"
        )]
        calibrate: bool,
        ...
        #[arg(
            long,
            help = "Surveyed base station ECEF coordinate override (x,y,z in meters)",
            group = "base_coord"
        )]
        base_coord_manual: Option<String>,
        #[arg(
            long,
            help = "Extract base coordinate from RTCM3 messages 1005/1006",
            group = "base_coord"
        )]
        base_coord_rtcm: bool,
        #[arg(
            long,
            help = "Automatically fetch official coordinate from NGS CORS API",
            group = "base_coord"
        )]
        base_coord_api: bool,
        #[arg(
            long,
            help = "Survey the base station using PPP before processing rover",
            group = "base_coord"
        )]
        base_coord_ppp: bool,
    ...
    Fetch {
        #[arg(
            long,
            help = "Path to rover observation file to determine location and time"
        )]
        rover_obs: String,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        Commands::Process {
            ...
            let mut rover_approx_pos: Option<[f64; 3]> = None;
            let mut rover_rinex_epochs = if rover.ends_with(".obs")
                || rover.ends_with("o")
                || rover.ends_with(".rnx")
                || rover.ends_with(".RNX")
            {
                let file = std::fs::File::open(&rover)?;
                let (epochs, rover_header) = gneiss_parsers::rinex::parse_rinex_obs(std::io::BufReader::new(file))?;
                info!("Loaded {} RINEX rover epochs.", epochs.len());
                if let Some(pos) = rover_header.approx_position {
                    info!("Rover APPROX POSITION: {:?}", pos);
                    rover_approx_pos = Some(pos);
                }
                Some(epochs)
            } else {
                ...
                if base_file.ends_with(".obs") || base_file.ends_with("o") {
                    ...
                    while !buffer.is_empty() {
                        if let Ok((rem, frame)) = gneiss_parsers::rtcm3::parse_rtcm3_frame(buffer) {
                            if let Ok(msg) =
                                gneiss_parsers::rtcm3::msm::parse_msm_message(frame.payload)
                            {
                                b_epochs.push(msg.into_epoch_obs());
                            } else if let Ok(arp) =
                                gneiss_parsers::rtcm3::station::parse_station_arp(frame.payload)
                            {
                                if base_coord_rtcm {
                                    approx_base_pos = Some([arp.ecef_x, arp.ecef_y, arp.ecef_z]);
                                    info!(
                                        "Extracted base coordinate from RTCM3 ARP: {:?}",
                                        approx_base_pos
                                    );
                                }
                            }
                    ...
                    let mut merged_epochs: std::collections::BTreeMap<
                        i64,
                        gneiss_core::obs::EpochObs,
            ...
            if base_coord_rtcm {
                ...
                if let Some(marker) = &base_marker_name {
                    ...
                    info!(
                        "Fetching official coordinate for {} from NOAA CORS API...",
                        marker
                    );
                    match provider.fetch_station_coordinate(marker).await {
                        Ok(coord) => {
                            approx_base_pos =
                                Some([coord.vector.x, coord.vector.y, coord.vector.z]);
                            info!(
                                "Base coordinate retrieved from NOAA API: {:?}",
                                approx_base_pos
                            );
                        }
                        Err(e) => {
                            return Err(format!(
                                "Failed to fetch coordinate for {}: {}",
                                marker, e
                            )
                            .into());
                        }
                    ...
                    return Err(
                        "No MARKER NAME found in base RINEX. Cannot fetch API coordinate.".into(),
                    );
                }
            ...
            if let Some(ls) = lambda_subset {
                engine_config.lambda_min_subset = ls;
            }

            if let Some(raim) = raim_outlier_m {
                engine_config.raim_pseudorange_outlier_m = raim;
            }
            if let Some(chi_pr) = chi_square_pr {
                engine_config.chi_square_pr_threshold = chi_pr;
            }
            if let Some(chi_cp) = chi_square_cp {
                engine_config.chi_square_cp_threshold = chi_cp;
            }
            if let Some(snr) = min_snr {
                engine_config.min_snr_dbhz = snr;
            }

            let arm_parts: Vec<f64> = lever_arm
                .split(',')
            ...
            if let Some(pos) = approx_base_pos {
                ...
                let cli_override = base_coord_manual.is_some()
                    || base_coord_rtcm
                    || base_coord_api
                    || base_coord_ppp;
            ...
            // IGS stations have cm-accurate coordinates in the header.
            ...
            if let (Some(r_epochs), Some((truth_tow, mut state))) =
                (&mut rover_rinex_epochs, initial_truth.clone())
            {
                if let Some(first_r) = r_epochs.first() {
                    time_offset = truth_tow - first_r.time.tow;
                    info!(
                        "Detected Time Offset: {:.3}s. Aligning Observations...",
                        time_offset
                    );
            ...
            if std::path::Path::new(&nav_file).exists() {
                if let Ok(file) = std::fs::File::open(&nav_file) {
                    match gneiss_parsers::rinex::parse_rinex_nav(std::io::BufReader::new(file)) {
                        Ok((ephemerides, klobuchar)) => {
                            ...
                            if let Some(klob) = klobuchar {
                                engine.klobuchar_params = Some(klob);
                            }
                        }
                ...
                tracing::warn!(
                    "Nav file {} does not exist. Engine may fall back to default or fail.",
                    nav_file
                );
            }
            ...
            if let Some(clk_path) = clk {
                if let Ok(content) = std::fs::read_to_string(&clk_path) {
                    ...
                    info!(
                        "Loaded CLK file with {} satellites.",
                        rinex_clk.satellites.len()
                    );
            ...
            if let Some(atx_file) = antex {
                match gneiss_parsers::antex::AntexDatabase::parse(&atx_file) {
                    Ok(db) => {
                        info!(
                            "Loaded ANTEX database from {} ({} antennas).",
                            atx_file,
                            db.antennas.len()
                        );
            ...
            for dcb_file in dcb {
                ...
                let dcb_type = if filename.starts_with("P1C1") {
                    "P1C1"
                } else if filename.starts_with("P2C2") {
                    "P2C2"
                } else if filename.starts_with("P1P2") {
                    "P1P2"
                } else {
                    "UNKNOWN"
                };
                ...
                if dcb_type != "UNKNOWN" {
                    if let Ok(content) = std::fs::read_to_string(&dcb_file) {
                        ...
                        info!(
                            "Loaded {} DCBs of type {} from {}",
                            count, dcb_type, dcb_file
                        );
                    } else {
                ...
                if let Ok(content) = std::fs::read_to_string(&imu_file_path) {
                    for line in content.lines().skip(1) {
                        ...
                        if p.len() >= 5 {
                            ...
                            imu_measurements.push(gneiss_core::imu::ImuMeasurement::new(
                                (tow * 1000.0) as u32,
                                accel_frd,
                                gyro_frd,
                            ));
            ...
            if calibrate_imu {
                ...
                let (roll, pitch) = gneiss_rtk::calibration::mounting::estimate_gravity_alignment(
                    &imu_measurements,
                )
                ...
                info!(
                    "Detected Mounting Offsets: Roll={:.2}°, Pitch={:.2}°",
                    roll_deg, pitch_deg
                );
            ...
            if calibrate {
                ...
                let eval_fn = |cfg: &EngineConfig| -> f64 {
                    ...
                    for r in rover_slice {
                        ...
                        while eval_imu_idx < imu_measurements.len()
                            && (imu_measurements[eval_imu_idx].time_tag as f64 / 1000.0)
                                <= current_tow
                        {
                ...
                if let Ok((best_lever_arm, best_angles)) =
                    gneiss_rtk::calibration::extrinsics::calibrate_extrinsics_6dof(
                        &engine_config,
                        eval_fn,
                    )
                {
                    ...
                    info!(
                        "Extrinsics calibrated. Lever Arm: {:?}, Mounting Angles: {:?}",
                        best_lever_arm, best_angles
                    );
                } else {
            ...
            for pass in 1..=passes {
                ...
                if let Some(r_epochs) = &mut rover_rinex_epochs {
                    ...
                    for r_ref in r_epochs.iter_mut() {
                        ...
                        while imu_idx < imu_measurements.len()
                            && (imu_measurements[imu_idx].time_tag as f64 / 1000.0) <= current_tow
                        {
                            engine.add_imu_measurement(imu_measurements[imu_idx].clone());
                            imu_idx += 1;
                        }

                        if let Err(e) = engine.process_epoch(&r, b) {
                            error!("Fail: {}", e);
                        } else {
                            processed_epochs += 1;
                        }
            ...
            for s in final_results {
                ...
                let line = format!(
                    ...
                    out_state.time.week,
                    out_state.time.tow,
                    out_state.position.vector.x,
                    out_state.position.vector.y,
                    out_state.position.vector.z,
                    if out_state.is_fixed { 1 } else { 2 }
                );
        ...
        Commands::Fetch {
            ...
            let (epochs, _header) = gneiss_parsers::rinex::parse_rinex_obs(std::io::Cursor::new(
                header_and_first_epoch,
            ))
