...
pub fn apply_state_correction(state: &mut RtkState, dx: &DVector<f64>) {
    ...
    if crate::filter::CORE_STATE_SIZE > 15 && state.covariance.nrows() > 15 {
...
pub fn apply_joseph_covariance_update(
    ...
    let identity = DMatrix::identity(state_cov.nrows(), state_cov.ncols());
    let i_kh = identity - k * h;
    let mut p_new = &i_kh * state_cov * i_kh.transpose() + k * r * k.transpose();
...
pub fn filter_pre_fit_residuals(
    ...
    for i in 0..z.len() {
        let nu = z[i];
        let h_row = h.row(i);
        let p_ht = state_cov * h_row.transpose();
        ...
        let threshold = match meas_type {
            ...
            3 => max_innovation * 1000.0,        // Doppler
...
pub fn update_loosely_coupled(
    ...
    let mut state_copy = dx.clone();
    // Use the apply_state_correction helper but modify attitude locally if it's not full state
    if state.covariance.nrows() < crate::filter::CORE_STATE_SIZE {
    ...
    state.covariance = apply_joseph_covariance_update(&state.covariance, &k, &h_mat, &DMatrix::from_diagonal(&DVector::from_iterator(6, [r_6x6[(0,0)], r_6x6[(1,1)], r_6x6[(2,2)], r_6x6[(3,3)], r_6x6[(4,4)], r_6x6[(5,5)]].into_iter())));
    // Wait, the loosely coupled update takes a full dense R_6x6 matrix, apply_joseph_covariance_update takes `&r` which is a matrix.
    // Let's pass the 6x6 R matrix.
    let mut full_r_6x6 = DMatrix::zeros(6, 6);
    for i in 0..6 {
        for j in 0..6 {
            full_r_6x6[(i, j)] = r_6x6[(i, j)];
        }
    }
    state.covariance = apply_joseph_covariance_update(&state.covariance, &k, &h_mat, &full_r_6x6);
...
pub fn update(state: &mut RtkState, z: &DVector<f64>, h: &DMatrix<f64>, r: &DMatrix<f64>, max_innovation: f64, meas_types: Option<&[(gneiss_core::sat::SatelliteId, u8)]>, is_tightly_coupled: bool) -> Result<Vec<usize>, UpdateError> {
    ...
    if total_pr > 0 && pr_valid_count == 0 {
        tracing::error!("EKF rejected all {} pseudoranges. Falling back to SPP.", total_pr);
    ...
    apply_state_correction(state, &dx);
    state.covariance = apply_joseph_covariance_update(&state.covariance, &k, &h_filt, &r_filt);
