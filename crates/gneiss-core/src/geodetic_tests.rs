...
mod tests {
    ...
    fn test_ecef_llh_round_trip_poles() {
        ...
        assert!(
            (llh_back.x - FRAC_PI_2).abs() < 1e-10,
            "Latitude error at North Pole: {}",
            (llh_back.x - FRAC_PI_2).abs()
        );
        assert!(
            llh_back.z.abs() < 1e-3,
            "Height error at North Pole: {}",
            llh_back.z.abs()
        );
        ...
        assert!(
            (llh_back.x - (-FRAC_PI_2)).abs() < 1e-10,
            "Latitude error at South Pole: {}",
            (llh_back.x + FRAC_PI_2).abs()
        );
        assert!(
            llh_back.z.abs() < 1e-3,
            "Height error at South Pole: {}",
            llh_back.z.abs()
        );
    ...
    fn test_ecef_llh_round_trip_high_altitude() {
        ...
        assert!(
            (llh_back.z - 35786000.0).abs() < 1e-3,
            "Height error at GEO: {}",
            (llh_back.z - 35786000.0).abs()
        );

        // GPS orbit altitude (~20200 km) at 45° latitude
        let gps_llh = Vector3::new(
    ...
    fn test_ecef_llh_round_trip_parametric() {
        ...
        for (lat, lon, height) in test_points {
            ...
            assert!(
                (llh_back.x - lat).abs() < 1e-9,
                "Lat round-trip failed for ({}, {}, {}): error = {}",
                lat,
                lon,
                height,
                (llh_back.x - lat).abs()
            ...
            let lon_err = ((llh_back.y - lon) + core::f64::consts::PI)
                .rem_euclid(2.0 * core::f64::consts::PI)
                - core::f64::consts::PI;
            assert!(
                lon_err.abs() < 1e-9,
                "Lon round-trip failed for ({}, {}, {}): error = {}",
                lat,
                lon,
                height,
                lon_err.abs()
            ...
            assert!(
                (llh_back.z - height).abs() < 1e-3,
                "Height round-trip failed for ({}, {}, {}): error = {}m",
                lat,
                lon,
                height,
                (llh_back.z - height).abs()
            );
        }
    }
}
