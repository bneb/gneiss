...
fn solve_eccentric_anomaly(mk: f64, e: f64) -> f64 {
    let mut ek = mk;
    for _ in 0..10 { ek = mk + e * libm::sin(ek); }
    ek
}

#[allow(clippy::too_many_arguments)]
fn compute_orbit_plane(ek: f64, e: f64, omega: f64, cus: f64, cuc: f64, crs: f64, crc: f64, cis: f64, cic: f64, a: f64, i0: f64, idot: f64, tk: f64) -> (f64, f64, f64) {
...
#[allow(clippy::too_many_arguments)]
fn rotate_to_ecef(xk_p: f64, yk_p: f64, vx_p: f64, vy_p: f64, omegak: f64, om_dot: f64, i: f64, idot: f64) -> (f64, f64, f64, f64, f64, f64) {
    let cos_ok = libm::cos(omegak); let sin_ok = libm::sin(omegak);
    let cos_i = libm::cos(i); let sin_i = libm::sin(i);
...
#[allow(clippy::too_many_arguments)]
fn apply_bds_geo_rotation(x: f64, y: f64, z: f64, vx: f64, vy: f64, vz: f64, tk: f64, omega_e: f64) -> (f64, f64, f64, f64, f64, f64) {
    let sin_5 = libm::sin(-5.0f64.to_radians()); let cos_5 = libm::cos(-5.0f64.to_radians());
    let sin_oet = libm::sin(omega_e * tk); let cos_oet = libm::cos(omega_e * tk);
    let xg = x * cos_oet + y * sin_oet * cos_5 + z * sin_oet * sin_5;
    let yg = -x * sin_oet + y * cos_oet * cos_5 + z * cos_oet * sin_5;
    let zg = -y * sin_5 + z * cos_5;
    let vxg = vx * cos_oet - x * omega_e * sin_oet + vy * sin_oet * cos_5 + y * omega_e * cos_oet * cos_5 + vz * sin_oet * sin_5 + z * omega_e * cos_oet * sin_5;
    let vyg = -vx * sin_oet - x * omega_e * cos_oet + vy * cos_oet * cos_5 - y * omega_e * sin_oet * cos_5 + vz * cos_oet * sin_5 - z * omega_e * sin_oet * sin_5;
...
fn calc_keplerian(
    ...
    let (u, r, i) = compute_orbit_plane(ek, e, omega, cus, cuc, crs, crc, cis, cic, a, i0, idot, tk);
    let (vx_p, vy_p) = compute_orbit_velocities(a, e, ek, n, u, r);
    
    let xk_p = r * libm::cos(u);
    let yk_p = r * libm::sin(u);

    let omegak = if is_bds_geo { omega0 + omega_dot * tk - omega_e * toe.tow } else { omega0 + (omega_dot - omega_e) * tk - omega_e * toe.tow };
    let om_dot = if is_bds_geo { omega_dot } else { omega_dot - omega_e };
    
    let (mut x, mut y, mut z, mut vx, mut vy, mut vz) = rotate_to_ecef(xk_p, yk_p, vx_p, vy_p, omegak, om_dot, i, idot);

    if is_bds_geo {
        let rot = apply_bds_geo_rotation(x, y, z, vx, vy, vz, tk, omega_e);
        x = rot.0; y = rot.1; z = rot.2; vx = rot.3; vy = rot.4; vz = rot.5;
    }
...
mod tests {
    ...
    fn test_ephemeris_exact() {
        ...
        macro_rules! assert_vec_eq {
            ($a:expr, $b:expr) => {
                assert!(($a - $b).norm() < 1e-6, "Vectors not equal: {:?} and {:?}", $a, $b);
            };
        }
