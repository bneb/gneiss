...
impl AtmosphereModel {
    ...
    ///
    /// Uses bilinear spatial interpolation on the VTEC grid and linear
    /// temporal interpolation between adjacent TEC maps.  The slant TEC
    /// is computed by applying a standard single-layer mapping function
    /// (MSLM) at the specified shell height.
    ///
    /// `tec_maps`: slice of TEC maps ordered by time (must be non-empty).
    /// `grid_lat1/lat2/dlat`: latitude grid parameters in degrees.
    /// `grid_lon1/lon2/dlon`: longitude grid parameters in degrees.
    /// `height_km`: ionospheric shell height in km (typically 450 km).
    /// `pos_llh`: receiver geodetic position [lat_rad, lon_rad, h_m].
    /// `az`: satellite azimuth in radians.
    /// `el`: satellite elevation in radians.
    /// `time`: GPS time of observation.
    ...
    pub fn iono_ionex(
        ...
        if tec_maps.is_empty() || el <= 0.0 {
            return 0.0;
        }
        // Guard against empty TEC grids
        if tec_maps[0].1.is_empty() || tec_maps[0].1[0].is_empty() {
            return 0.0;
        }

        let re = 6371.0; // Earth radius in km
        let h = height_km;

        // --- Ionospheric pierce point (IPP) computation ---
        // Single-layer model: the IPP is where the receiver-satellite LOS
        // intersects a thin spherical shell at height_km above the Earth.
        let psi = (libm::asin(re / (re + h) * libm::cos(el)) - (core::f64::consts::PI / 2.0 - el))
            .max(0.0);
        ...
        .to_degrees();
        let ipp_lon = lon_r
            + libm::asin(libm::sin(psi) * libm::sin(az) / libm::cos(ipp_lat.to_radians()))
                .to_degrees();

        // Clamp IPP to grid bounds
        let ipp_lat = ipp_lat.clamp(grid_lat1.min(grid_lat2), grid_lat1.max(grid_lat2));
        let ipp_lon = ipp_lon.clamp(grid_lon1.min(grid_lon2), grid_lon1.max(grid_lon2));

        // --- Temporal interpolation ---
        // Find the two TEC maps bracketing `time`.
        let n = tec_maps.len();
        let t1_idx: usize;
        let t2_idx: usize;
        let mut frac = 0.0f64;

        // Convert GPS times to seconds-of-week for comparison
        let tow = time.tow;
        let tow0 = tec_maps[0].0.tow;
        let tow_last = tec_maps[n - 1].0.tow;

        if tow <= tow0 {
            t1_idx = 0;
            t2_idx = 0;
            frac = 0.0;
        } else if tow >= tow_last {
            t1_idx = n - 1;
            t2_idx = n - 1;
            frac = 0.0;
        } else {
            ...
            match tec_maps.binary_search_by(|m| m.0.tow.partial_cmp(&tow).expect("TEC map TOW is never NaN")) {
                ...
                Err(idx) => {
                    if idx == 0 {
                        ...
                        t1_idx = idx - 1;
                        t2_idx = idx;
                        let dt = tec_maps[t2_idx].0.tow - tec_maps[t1_idx].0.tow;
                        if dt > 0.0 {
                            frac = (tow - tec_maps[t1_idx].0.tow) / dt;
                        }
                    }
                }
            }
        ...
        let tec_at = |map: &Vec<Vec<f64>>, lat: f64, lon: f64| -> f64 {
            let nlat = map.len();
            if nlat == 0 {
                return 0.0;
            }
            ...
            // Longitude index (fractional, wrap-around)
        ...
        let vtec1 = tec_at(tec_maps[t1_idx].1, ipp_lat, ipp_lon);
        let vtec2 = tec_at(tec_maps[t2_idx].1, ipp_lat, ipp_lon);
        let vtec = vtec1 * (1.0 - frac) + vtec2 * frac;

        // --- Obliquity factor (MSLM mapping function) ---
        let chi = libm::asin(re / (re + h) * libm::cos(el));
        let mf = 1.0 / libm::cos(chi);

        // --- Convert VTEC to slant delay ---
        // STEC = VTEC * mf  (in TECU = 10^16 el/m^2)
        // Ionospheric delay on L1: I = 40.3 * STEC / f1^2
        // = 40.3e16 * VTEC * mf / f1^2
        // where f1 = 1575.42e6 Hz for GPS L1.
        // 40.3e16 / (1575.42e6)^2 = 40.3e16 / 2.4819e18 ≈ 0.1624 m/TECU
        let delay_m = 0.1624 * vtec * mf;

        // Clamp negative delays (shouldn't happen, but safety)
        delay_m.max(0.0)
    }
