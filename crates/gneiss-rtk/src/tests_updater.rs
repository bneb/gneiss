...
mod tests {
    ...
    fn test_loosely_coupled_jacobian() {
        ...
        for j in 0..3 {
            ...
            dpsi_pos[j] = epsilon;
            state_pos.attitude =
                nalgebra::UnitQuaternion::from_scaled_axis(dpsi_pos) * state_pos.attitude;

            let mut state_neg = state.clone();
            let mut dpsi_neg = Vector3::zeros();
            dpsi_neg[j] = -epsilon;
            state_neg.attitude =
                nalgebra::UnitQuaternion::from_scaled_axis(dpsi_neg) * state_neg.attitude;

            let pos_apc_pos =
                state_pos.position.vector + state_pos.attitude.to_rotation_matrix() * lever_arm;
            let pos_apc_neg =
                state_neg.position.vector + state_neg.attitude.to_rotation_matrix() * lever_arm;
            let num_pos_att = (pos_apc_pos - pos_apc_neg) / (2.0 * epsilon);

            assert!((num_pos_att - h_pos_att.column(j)).norm() < 1e-5);

            let v_apc_pos = state_pos.velocity
                + state_pos.attitude.to_rotation_matrix() * omega_b.cross(&lever_arm);
            let v_apc_neg = state_neg.velocity
                + state_neg.attitude.to_rotation_matrix() * omega_b.cross(&lever_arm);
        ...
        for j in 0..3 {
            ...
            let v_apc_pos = state.velocity
                + state.attitude.to_rotation_matrix() * omega_b_pos.cross(&lever_arm);
            let v_apc_neg = state.velocity
                + state.attitude.to_rotation_matrix() * omega_b_neg.cross(&lever_arm);
