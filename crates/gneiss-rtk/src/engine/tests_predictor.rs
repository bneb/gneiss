...
mod tests {
    ...
    fn test_physics_stationary_gravity() {
        ...
        assert!(
            ...
            "Stationary IMU should not gain velocity, got {}",
            state.velocity.norm()
        );
    ...
    fn test_physics_centrifugal_cancellation() {
        ...
        assert!(
            state.velocity.norm() < 1e-3,
            "Equatorial stationary IMU should be stable, got {}",
            state.velocity.norm()
        );
    }
    ...
    fn test_coupling_lever_arm_to_attitude() {
        ...
        for i in 6..9 {
            state.covariance[(i, i)] = 1.0f64.to_radians().powi(2);
        }
        ...
        assert!(
            dx[8].abs() > 1e-6,
            "GNSS range should correct attitude through lever arm, got {}",
            dx[8]
        );
    }
