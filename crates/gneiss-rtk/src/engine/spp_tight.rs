...
pub fn process_spp_tightly_coupled<'a>(engine: &'a mut ProcessingEngine, rover_obs: &'a EpochObs) -> Result<&'a RtkState, EngineError> {
    ...
    let measurements: Vec<_> = build_measurements(rover_obs, &engine.ephemerides, &SppConfig::default())
        .into_iter()
        .filter(|m| m.snr >= 25.0)
        .collect();
...
fn bootstrap_clock_bias(engine: &mut ProcessingEngine, rover_obs: &EpochObs) {
    if let Ok(spp_res) = crate::spp::compute_spp(rover_obs, &engine.ephemerides, engine.klobuchar_params.as_ref(), &SppConfig::default(), None) {
        let state = engine.current_state.as_mut().unwrap();
...
fn get_apc_kinematics(engine: &ProcessingEngine) -> (Vector3<f64>, Vector3<f64>, nalgebra::Rotation3<f64>, Vector3<f64>, Vector3<f64>) {
    ...
    let omega_b = engine.imu_history.last().and_then(|buf: &Vec<gneiss_core::imu::ImuMeasurement>| buf.last())
...
struct EkfContext<'a> {
    pos_apc: Vector3<f64>, v_apc: Vector3<f64>, r_b_e: nalgebra::Rotation3<f64>,
    lever_arm: Vector3<f64>, omega_eb_b: Vector3<f64>, rec_llh: Vector3<f64>,
...
fn build_ekf_matrices(engine: &ProcessingEngine, measurements: &[SppMeasurement]) -> (DVector<f64>, DMatrix<f64>, DMatrix<f64>, Vec<(gneiss_core::sat::SatelliteId, u8)>) {
    ...
    let ctx = EkfContext {
        pos_apc, v_apc, r_b_e, lever_arm, omega_eb_b,
        rec_llh: ecef_to_llh(pos_apc), state: engine.current_state.as_ref().unwrap(),
        engine, n_cols: engine.current_state.as_ref().unwrap().covariance.ncols(),
    };
...
fn handle_rejection(engine: &mut ProcessingEngine, rover_obs: &EpochObs, rejected: bool) -> Result<(), EngineError> {
    let state = engine.current_state.as_mut().unwrap();
    if rejected {
...
fn reset_state_from_spp(engine: &mut ProcessingEngine, rover_obs: &EpochObs) -> Result<(), EngineError> {
    let spp_res = crate::spp::compute_spp(rover_obs, &engine.ephemerides, engine.klobuchar_params.as_ref(), &SppConfig::default(), None)
    ...
    for i in 0..3 { state.covariance[(i, i)] = COV_POS_RESET; }
    for i in 3..6 { state.covariance[(i, i)] = COV_VEL_RESET; }
