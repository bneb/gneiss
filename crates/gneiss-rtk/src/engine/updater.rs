...
fn build_loose_coupling_h(state: &RtkState, lever_arm: &Vector3<f64>, omega_b: &Vector3<f64>) -> DMatrix<f64> {
...
pub fn update_loosely_coupled(
    ...
    let r_6x6 = huber_scale_covariance(&p_6x6, &r_6x6_raw, &z, tuning.loosely_coupled_mahalanobis_sq)
...
fn subset_ekf_matrices(
    ...
    for (new_idx, &old_idx) in valid_indices.iter().enumerate() {
        ...
        for j in 0..cols { h_new[(new_idx, j)] = h[(old_idx, j)]; }
...
fn check_outlier(
    ...
    *current_r = current_r.clone().remove_row(worst_idx).remove_column(worst_idx);
    *base_r = base_r.clone().remove_row(worst_idx).remove_column(worst_idx);
...
fn run_ekf_iterations<C: CouplingStrategy>(
    ...
    for iter in 0..100 {
        ...
        if let Some(idx) = res.worst_idx {
            match check_outlier(current_valid, current_z, current_h, current_r, base_r, idx, iter, res.max_outlier_ratio)? {
...
fn validate_ekf_dimensions(z: &DVector<f64>, h: &DMatrix<f64>, cov_cols: usize) -> Result<(), UpdateError> {
...
fn validate_pr_measurements(
    ...
    let pr_count = valid_indices.iter()
...
pub fn update<C: CouplingStrategy>(
    ...
    let (dx, k) = run_ekf_iterations::<C>(
        state, &mut cz, &mut ch, &mut cr,
        &mut current_valid, &mut base_r,
        meas_types, chi_square_threshold, tuning,
    )?;
    apply_state_correction(state, &dx);
