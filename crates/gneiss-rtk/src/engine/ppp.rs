use gneiss_core::sat::Constellation;use gneiss_core::obs::EpochObs;
...
fn build_sats<'a>(engine: &ProcessingEngine, r_obs: &'a EpochObs) -> Vec<ProcessedSat<'a>> {
    let state = engine.current_state.as_ref().unwrap();
    let base_rcv_pos = Vector3::new(state.position.vector.x, state.position.vector.y, state.position.vector.z);
    let rcv_pos = base_rcv_pos + gneiss_core::tides::solid_earth_tides_ecef(r_obs.time, base_rcv_pos);
...
fn get_obs_and_corrections(
    ...
    let osb = crate::engine::ppp_math::apply_osb_corrections(engine.sinex_bias.as_ref(), sat_obs, time, f1, f2, f1_b, f2_b);
...
fn compute_sat_state(
    engine: &ProcessingEngine, eph: &gneiss_core::ephemeris::Ephemeris, sat: gneiss_core::sat::SatelliteId, t_nom: gneiss_core::time::GpsTime
    ...
    let mut sat_pos: Vector3<f64> = brdc_pos; let mut sat_vel: Vector3<f64> = brdc_vel;
...
"#;
        ...
        let f1 = 1575.42e6; let wl1 = LIGHT_SPEED / f1;
        let f2 = 1227.60e6; let wl2 = LIGHT_SPEED / f2;

        let mut obs = SatObs { sat: SatelliteId { constellation: Constellation::Gps, prn: 1 }, observations: vec![] };
        obs.sat = sat;
        obs.observations.push(gneiss_core::obs::Observation { code: ObsCode::from_str("C1C").unwrap(), value: 10.0, lli: None, lock_time: None });
        obs.observations.push(gneiss_core::obs::Observation { code: ObsCode::from_str("C2L").unwrap(), value: 20.0, lli: None, lock_time: None });
        obs.observations.push(gneiss_core::obs::Observation { code: ObsCode::from_str("L1C").unwrap(), value: 30.0, lli: None, lock_time: None });
        obs.observations.push(gneiss_core::obs::Observation { code: ObsCode::from_str("L2L").unwrap(), value: 40.0, lli: None, lock_time: None });

        let res = crate::engine::ppp_math::apply_osb_corrections(Some(&bias), &obs, GpsTime::new(2156, 129600.0), f1, f2, 1, 2);
...
mod ppp_tests {
    use super::*;
    use crate::engine::{ProcessingEngine, EngineConfig};
    ...
    fn test_valid_pos() {
        ...
        let mut state = RtkState::new(
            ...
            Coordinate::new(Vector3::new(0.0, 0.0, f64::NAN), Datum::WGS84, Frame::ECEF, gneiss_core::time::GpsTime::new(0, 0.0)),
            1.0
        );
    ...
    fn test_process_ppp_empty_sats() {
        ...
        let mut state = RtkState::new(
            ...
            Coordinate::new(Vector3::new(2000.0, 2000.0, 2000.0), Datum::WGS84, Frame::ECEF, gneiss_core::time::GpsTime::new(2156, 129000.0)),
            1.0
        );
        ...
        let obs = EpochObs { time, satellites: vec![] };
    ...
    fn test_process_single_sat() {
        ...
        use gneiss_core::obs::{SatObs, Observation, ObsCode};
        use std::str::FromStr;
        ...
        let eph = Ephemeris::Gps(gneiss_core::ephemeris::GpsEphemeris {
            sat, toe: t, toc: t, af0: 1e-5, af1: 0.0, af2: 0.0,
            crs: 0.0, crc: 0.0, cuc: 0.0, cus: 0.0, cic: 0.0, cis: 0.0,
            m0: 0.0, e: 0.0, sqrt_a: 5500.0, delta_n: 0.0,
            omega0: 0.0, omega_dot: 0.0, i0: std::f64::consts::PI / 4.0, idot: 0.0, omega: 0.0, tgd: 0.0,
            iode: 0, iodc: 0,
        });
        engine.ephemerides.push(eph);

        let mut sat_obs = SatObs { sat, observations: vec![] };
        sat_obs.observations.push(Observation { code: "C1C".parse().unwrap(), value: crate::engine::ppp_math::LIGHT_SPEED * 0.07, lli: None, lock_time: None });
        sat_obs.observations.push(Observation { code: "L1C".parse().unwrap(), value: 1000.0, lli: None, lock_time: None });
        ...
        assert_eq!(psat.f1, gneiss_core::signal::satellite_frequencies(sat, 0).0);
        assert_eq!(psat.f2, gneiss_core::signal::satellite_frequencies(sat, 0).1);
        ...
        assert_eq!(psat.lam1, crate::engine::ppp_math::LIGHT_SPEED / gneiss_core::signal::satellite_frequencies(sat, 0).0);
        assert_eq!(psat.lam2, crate::engine::ppp_math::LIGHT_SPEED / gneiss_core::signal::satellite_frequencies(sat, 0).1);
    }
    ...
    fn test_process_single_sat_low_el() {
        ...
        let sat = SatelliteId { constellation: Constellation::Gps, prn: 1 };
        let eph = Ephemeris::Gps(gneiss_core::ephemeris::GpsEphemeris {
            sat, toe: t, toc: t, af0: 0.0, af1: 0.0, af2: 0.0,
            crs: 0.0, crc: 0.0, cuc: 0.0, cus: 0.0, cic: 0.0, cis: 0.0,
            m0: 0.0, e: 0.0, sqrt_a: 5500.0, delta_n: 0.0,
    ...
    fn test_get_obs_and_corrections() {
        ...
        let sat = SatelliteId { constellation: Constellation::Gps, prn: 1 };
        let mut sat_obs = SatObs { sat, observations: vec![] };
        sat_obs.observations.push(Observation { code: "C1C".parse().unwrap(), value: 1000.0, lli: None, lock_time: None });
        sat_obs.observations.push(Observation { code: "C2W".parse().unwrap(), value: 2000.0, lli: None, lock_time: None });
        sat_obs.observations.push(Observation { code: "L1C".parse().unwrap(), value: 3000.0, lli: None, lock_time: None });
        sat_obs.observations.push(Observation { code: "L2W".parse().unwrap(), value: 4000.0, lli: None, lock_time: None });

        let (p1, p2, cp1, cp2, _, is_if) = get_obs_and_corrections(&engine, &sat_obs, t, 1.5e9, 1.2e9);
