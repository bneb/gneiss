...
pub fn compute_phase_windup(
    ...
    let w_bas_sat = gneiss_core::windup::phase_windup(bas_pos_sat, sun_pos, base_coord_vec, prev_w_bas_sat);
    let w_bas_ref = gneiss_core::windup::phase_windup(bas_pos_ref, sun_pos, base_coord_vec, prev_w_bas_ref);

    WindupUpdates { w_sat, w_ref, w_bas_sat, w_bas_ref }
}
...
mod tests {
    ...
    fn test_compute_phase_windup() {
        ...
        let exp_w_sat = gneiss_core::windup::phase_windup(Vector3::new(100.0, 0.0, 0.0), Vector3::new(1e11, 0.0, 0.0), Vector3::new(10.0, 20.0, 30.0), 0.1);
        assert!((updates.w_sat - exp_w_sat).abs() < 1e-9 || (updates.w_sat.is_nan() && exp_w_sat.is_nan()));
    }
...
pub fn build_dense_covariance_matrix(
    ...
    let mut r_mat = nalgebra::DMatrix::from_diagonal(&nalgebra::DVector::from_row_slice(r_diagonals));
    ...
    fn test_get_sat_state_catches_tau_pr_mutation() {
        ...
        let eph = Ephemeris::Gps(gneiss_core::ephemeris::GpsEphemeris {
            sat, toe: time, toc: time, af0: 0.0, af1: 0.01, af2: 0.0,
            crs: 0.0, crc: 0.0, cuc: 0.0, cus: 0.0, cic: 0.0, cis: 0.0, m0: 1.0, e: 0.01, sqrt_a: 5153.6, delta_n: 0.0, 
        ...
        // If tau_pr was added instead of subtracted, the time would shift by 2 * tau_pr
        // dt_s = af1 * (t - toc). If t changes by 0.133s, dt_s changes by 0.00133s
        // This will shift the true satellite transmission time by ~1ms, changing pos by ~3 meters
        let (pos2, _) = get_sat_state(&eph, -pr, time, rx_pos); // Emulate adding tau_pr
        
        assert!((pos1 - pos2).norm() > 1.0);
    }
