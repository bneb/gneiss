...
mod tests {
    ...
    fn test_update_loosely_coupled_exact_math() {
        ...
        assert!(state.attitude.vector().norm() > 0.0, "Attitude should be updated via jacobian");
    }
    ...
    fn test_compute_update_iteration_math() {
        ...
        let res = crate::engine::updater::compute_update_iteration::<crate::engine::updater::TightCoupling>(&state_cov, &z, &h, &r, &valid, None, 10.0, &tuning).unwrap();
        ...
        assert!(res.max_outlier_ratio < 1.0, "v should be 1.0, ratio 1.0/sqrt(5.0) ~ 0.44. If mutant +, v=9.0, ratio ~4.0");
    }
    ...
    fn test_compute_update_iteration_singular_s() {
        ...
        let res = crate::engine::updater::compute_update_iteration::<crate::engine::updater::TightCoupling>(&state_cov, &z, &h, &r, &valid, None, 10.0, &tuning);
        assert!(res.is_err(), "Should return Err for Inf in S");

        let state_cov_nan = DMatrix::from_diagonal(&DVector::from_element(1, f64::NAN));
        let res2 = crate::engine::updater::compute_update_iteration::<crate::engine::updater::TightCoupling>(&state_cov_nan, &z, &h, &r, &valid, None, 10.0, &tuning);
        assert!(res2.is_err(), "Should return Err for NaN in S");
        
        let state_cov_large = DMatrix::from_diagonal(&DVector::from_element(1, 2e15));
        let res3 = crate::engine::updater::compute_update_iteration::<crate::engine::updater::TightCoupling>(&state_cov_large, &z, &h, &r, &valid, None, 10.0, &tuning);
    ...
    fn test_compute_update_iteration_s_threshold() {
        ...
        let tuning = crate::engine::config::EkfTuningConfig::default();
        let res = crate::engine::updater::compute_update_iteration::<crate::engine::updater_math::LooseCoupling>(
            &state_cov, &current_z, &current_h, &current_r, &[0], None, 3.0, &tuning
        );
        assert!(res.is_ok());
    }
    ...
    fn test_apply_fix_and_hold_mutants() {
        ...
        let sat1 = gneiss_core::sat::SatelliteId { constellation: gneiss_core::sat::Constellation::Gps, prn: 1 };
        let sat2 = gneiss_core::sat::SatelliteId { constellation: gneiss_core::sat::Constellation::Gps, prn: 2 };
