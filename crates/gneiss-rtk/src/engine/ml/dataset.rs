...
pub fn export_epoch_to_csv(
    ...
    let sat_to_residual = extract_post_fit_residuals(innovations, h_matrix, dx, valid_indices, measurement_types);
    ...
    for (rov, _) in matched_obs {
        if let Some(&r_sq) = sat_to_residual.get(&rov.sat) {
            ...
            writeln!(file, "{},{},{:.2},{:.2},{:.2},{:.4},{:.0},{:.6}", 
                epoch_num, rov.sat, rov.snr, el, az, rov.doppler, rov.locktime.unwrap_or(0), r_sq
            ).unwrap();
        }
    }
}
