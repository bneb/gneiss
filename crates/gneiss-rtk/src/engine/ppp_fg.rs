...
impl PppFactorGraph {
    ...
    fn build_measurements(&self, state: &RtkState, sats: &[ProcessedSat], x_i: &DVector<f64>, iter: usize) -> Vec<FgMeasurement> {
        ...
        for sat in sats {
            ...
            self.push_pr_measurement(&mut meas, state, sat, x_i, iter, &los, expected_base, dist, isb);
            if sat.doppler != 0.0 {
                self.push_doppler_measurement(&mut meas, sat, x_i, &los);
            }
            if let Some(cp1) = sat.cp1 {
                if cp1 != 0.0 {
                    self.push_cp_measurement(&mut meas, state, sat, x_i, iter, &los, expected_base, dist, cp1);
                }
            }
        }
        meas
    ...
    fn push_pr_measurement(&self, meas: &mut Vec<FgMeasurement>, state: &RtkState, sat: &ProcessedSat, x_i: &DVector<f64>, iter: usize, los: &Vector3<f64>, expected_base: f64, dist: f64, isb: f64) {
        ...
        if res_pr.abs() > 1000.0 && iter == 0 {
            tracing::warn!("HUGE res_pr: sat={}, meas={:.2}, exp={:.2}, res={:.2}, dist={:.2}, clk={:.2}, isb={:.2}", 
                sat.sat_obs.sat, sat.p_meas, expected_pr, res_pr, dist, x_i[15], isb);
        }
