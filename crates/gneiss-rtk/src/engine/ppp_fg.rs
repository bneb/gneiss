...
impl PppFactorGraph {
    ...
    pub fn solve(&self, state: &mut RtkState, sats: &[ProcessedSat]) -> Result<(), EngineError> {
        for outer_iter in 0..4 {
            ...
            if done || outer_iter == 3 {
                if !done { tracing::warn!("Max outlier rejection iterations reached."); }
                break;
            }
        }
    ...
    pub fn resolve_cascade_ar(&self, state: &mut RtkState, sats: &[ProcessedSat]) -> Result<(), &'static str> {
        let cands = self.find_ar_candidates(state, sats);
        if cands.len() < 5 { return Err("Insufficient dual-frequency satellites for AR"); }
        let subset = self.build_ar_subset(&cands);
        if subset.len() < 4 { return Err("Insufficient satellites after single differencing"); }

        let x = extract_state_vector(state);
        let (x_wl, p_wl) = self.resolve_widelane_ar(state, &subset, &x)?;
        
        let (x_fixed, p_fixed) = self.resolve_narrowlane_ar(state, &subset, &x_wl, &p_wl)?;
    ...
    fn find_ar_candidates(&self, state: &RtkState, sats: &[ProcessedSat]) -> Vec<(gneiss_core::sat::SatelliteId, usize, usize, f64, f64, f64)> {
        let mut cands = Vec::new();
        for sat in sats.iter().filter(|s| !s.is_iono_free && s.cp2.is_some() && s.sat_obs.sat.constellation == gneiss_core::sat::Constellation::Gps) {
            if let (Some(n1), Some(n2)) = (find_amb_idx(state, sat.sat_obs.sat, 1), find_amb_idx(state, sat.sat_obs.sat, 2)) {
    ...
    fn build_ar_subset(&self, cands: &[(gneiss_core::sat::SatelliteId, usize, usize, f64, f64, f64)]) -> Vec<((gneiss_core::sat::SatelliteId, usize, usize, f64, f64, f64), (gneiss_core::sat::SatelliteId, usize, usize, f64, f64, f64))> {
        vec![]
    }

    fn resolve_widelane_ar(&self, state: &RtkState, subset: &[((gneiss_core::sat::SatelliteId, usize, usize, f64, f64, f64), (gneiss_core::sat::SatelliteId, usize, usize, f64, f64, f64))], x: &DVector<f64>) -> Result<(DVector<f64>, DMatrix<f64>), &'static str> {
        let mut d_wl_full = DMatrix::zeros(subset.len(), state.covariance.nrows());
        for (i, (c, ref_sat)) in subset.iter().enumerate() {
            d_wl_full[(i, CORE_STATE_SIZE + c.1)] = 1.0 / c.4; d_wl_full[(i, CORE_STATE_SIZE + c.2)] = -1.0 / c.5;
            d_wl_full[(i, CORE_STATE_SIZE + ref_sat.1)] = -1.0 / ref_sat.4; d_wl_full[(i, CORE_STATE_SIZE + ref_sat.2)] = 1.0 / ref_sat.5;
        }
        ...
        for (i, &idx) in keep_indices.iter().enumerate() { for j in 0..state.covariance.nrows() { d_wl[(i, j)] = d_wl_full[(idx, j)]; } }
        ...
        Ok((x + dx_wl, crate::math::covariance::apply_joseph_covariance_update(&state.covariance, &k_wl, &d_wl, &DMatrix::zeros(keep_indices.len(), keep_indices.len()))))
    }

    fn resolve_narrowlane_ar(&self, state: &RtkState, subset: &[((gneiss_core::sat::SatelliteId, usize, usize, f64, f64, f64), (gneiss_core::sat::SatelliteId, usize, usize, f64, f64, f64))], x_wl: &DVector<f64>, p_wl: &DMatrix<f64>) -> Result<(DVector<f64>, DMatrix<f64>), &'static str> {
        let mut d_nl = DMatrix::zeros(subset.len(), state.covariance.nrows());
        for (i, (c, ref_sat)) in subset.iter().enumerate() {
        ...
        let res_nl = crate::ambiguity::lambda::resolve_lambda(&a_nl, &q_nl).map_err(|_| "NL LAMBDA Failed")?;
        
        if res_nl.ratio < 3.0 || res_nl.success_rate < 0.99 { return Err("NL ratio test failed"); }

        let s_nl_inv = q_nl.try_inverse().ok_or("NL Cov Inversion failed")?;
        let mut k_nl = p_wl * d_nl.transpose() * s_nl_inv;
        for i in 6..15 { for j in 0..k_nl.ncols() { k_nl[(i, j)] = 0.0; } }
        ...
        tracing::info!("r dims: {}x{}", subset.len(), subset.len());

        let p_fixed = crate::math::covariance::apply_joseph_covariance_update(p_wl, &k_nl, &d_nl, &DMatrix::zeros(subset.len(), subset.len()));
        
        tracing::info!("PPP Cascade AR Fixed! N_Sats: {}", subset.len() + 1);
    ...
    fn push_pr_measurement(&self, meas: &mut Vec<FgMeasurement>, state: &RtkState, sat: &ProcessedSat, x_i: &DVector<f64>, iter: usize, los: &Vector3<f64>, expected_base: f64, _dist: f64, _isb: f64) {
        let expected_pr = if sat.is_iono_free { expected_base } else { expected_base + sat.iono_delay };
        let res_pr = sat.p1 - expected_pr;
        ...
        meas.push(FgMeasurement { res: res_pr, h_row: build_h_row(&los, sat.map_wet, None, x_i.len(), sat.sat_obs.sat.constellation), weight: var_pr / w_pr, raw_var: var_pr, is_phase: false, sat: Some(sat.sat_obs.sat) });
    }

    fn push_doppler_measurement(&self, meas: &mut Vec<FgMeasurement>, sat: &ProcessedSat, x_i: &DVector<f64>, los: &Vector3<f64>) {
        ...
        meas.push(FgMeasurement { res: res_rr, h_row: build_h_row_doppler(&los, x_i.len()), weight: var_rr / w_rr, raw_var: var_rr, is_phase: false, sat: Some(sat.sat_obs.sat) });
    }

    fn push_cp_measurement(&self, meas: &mut Vec<FgMeasurement>, state: &RtkState, sat: &ProcessedSat, x_i: &DVector<f64>, iter: usize, los: &Vector3<f64>, expected_base: f64, dist: f64, cp1: f64) {
        if let Some(amb_idx) = find_ambiguity_index(state, sat.sat_obs.sat) {
            ...
            let expected_cp = if sat.is_iono_free { expected_base + x_i[CORE_STATE_SIZE + amb_idx] } else { expected_base - sat.iono_delay + x_i[CORE_STATE_SIZE + amb_idx] };
            let res_cp = l_meas - expected_cp;
            if res_cp.abs() > 100.0 && iter == 0 { tracing::warn!("HUGE res_cp: sat={}, l_meas={:.2}, exp={:.2}, dist={:.2}, clk={:.2}, n_amb={:.2}", sat.sat_obs.sat, l_meas, expected_cp, dist, x_i[15], x_i[CORE_STATE_SIZE + amb_idx]); }
            let mut var_cp = 0.0001 * snr_scale(sat.snr as i32) / libm::sin(sat.el);
            if sat.is_iono_free { var_cp *= 9.0; } // Iono-free amplifies phase noise
            let w_cp = apply_huber(res_cp, var_cp, self.huber_k);
            meas.push(FgMeasurement { res: res_cp, h_row: build_h_row(&los, sat.map_wet, Some(CORE_STATE_SIZE + amb_idx), x_i.len(), sat.sat_obs.sat.constellation), weight: var_cp / w_cp, raw_var: var_cp, is_phase: true, sat: Some(sat.sat_obs.sat) });
        }
    }

    fn push_uduc_measurements(&self, meas: &mut Vec<FgMeasurement>, state: &RtkState, sat: &ProcessedSat, x_i: &DVector<f64>, _iter: usize, los: &Vector3<f64>, expected_base: f64, _dist: f64, _isb: f64) {
...
fn find_amb_idx(state: &RtkState, sat: gneiss_core::sat::SatelliteId, f: u8) -> Option<usize> {
    state.ambiguity_keys.iter().position(|&x| x == (sat, f))
}

fn build_h_row_uduc(los: &Vector3<f64>, map_wet: f64, i_idx: Option<usize>, i_coef: f64, n_idx: Option<usize>, size: usize, constel: gneiss_core::sat::Constellation) -> DVector<f64> {
    ...
    if let Some(idx) = i_idx { h[idx] = i_coef; }
    if let Some(idx) = n_idx { h[idx] = 1.0; }
    h
}
...
mod mutant_killer_tests {
    ...
    fn test_resolve_widelane_ar_insufficient() {
        ...
        for i in 0..4 { state.covariance[(CORE_STATE_SIZE + i, CORE_STATE_SIZE + i)] = 100.0; } // huge variance
        let subset = [
            ((SatelliteId { constellation: Constellation::Gps, prn: 1 }, 0, 0, 1.0, 1.0, 1.0), (SatelliteId { constellation: Constellation::Gps, prn: 2 }, 1, 1, 1.0, 1.0, 1.0)),
            ((SatelliteId { constellation: Constellation::Gps, prn: 1 }, 0, 0, 1.0, 1.0, 1.0), (SatelliteId { constellation: Constellation::Gps, prn: 3 }, 2, 2, 1.0, 1.0, 1.0)),
            ((SatelliteId { constellation: Constellation::Gps, prn: 1 }, 0, 0, 1.0, 1.0, 1.0), (SatelliteId { constellation: Constellation::Gps, prn: 4 }, 3, 3, 1.0, 1.0, 1.0)),
        ];
    ...
    fn test_push_cp_measurement_iono_free() {
        ...
        let sat = ProcessedSat {
            sat_obs: &obs,
            dt_sat_m: 0.0, p1: 0.0, p2: None, cp1: None, cp2: None,
            is_iono_free: true, osb_p1: 0.0, osb_p2: 0.0, osb_cp1: 0.0, osb_cp2: 0.0,
            los: Vector3::zeros(), dist: 0.0, el: std::f64::consts::PI / 2.0, snr: 45.0, doppler: 0.0,
        ...
        fg.push_cp_measurement(&mut meas, &state, &sat, &x_i, 0, &los, 10.0, 20000000.0, 100.0);
    ...
    fn test_resolve_cascade_ar_bounds() {
        ...
        let obs4 = SatObs { sat: SatelliteId { constellation: Constellation::Gps, prn: 4 }, observations: vec![] };
        let obs5 = SatObs { sat: SatelliteId { constellation: Constellation::Gps, prn: 5 }, observations: vec![] };
...

        {
            let mut add_sat = |obs: &'static SatObs| {
                state.ambiguity_keys.push((obs.sat, 1));
                state.ambiguity_keys.push((obs.sat, 2));
                state.ambiguities.push(0.0);
                state.ambiguities.push(0.0);
            ...
            add_sat(obs1_ref); add_sat(obs2_ref); add_sat(obs3_ref); add_sat(obs4_ref);
        }
        assert_eq!(fg.resolve_cascade_ar(&mut state, &sats), Err("Insufficient dual-frequency satellites for AR"));

        state.ambiguity_keys.push((obs5_ref.sat, 1));
        state.ambiguity_keys.push((obs5_ref.sat, 2));
        state.ambiguities.push(0.0);
        state.ambiguities.push(0.0);
        sats.push(ProcessedSat {
            sat_obs: obs5_ref,
        ...
        assert!(fg.resolve_cascade_ar(&mut state, &sats) != Err("Insufficient dual-frequency satellites for AR"));
    }
    ...
    fn test_find_worst_outlier() {
        ...
        let meas = vec![
            ...
            FgMeasurement { res: 1000.0, raw_var: 1.0, is_phase: false, sat: Some(sat1), h_row: DVector::zeros(0), weight: 1.0 },
            // Phase, but norm = 10.0 / sqrt(4.0) = 5.0 (less than max_norm 15.0)
            FgMeasurement { res: 10.0, raw_var: 4.0, is_phase: true, sat: Some(sat1), h_row: DVector::zeros(0), weight: 1.0 },
            // Phase, norm = 40.0 / sqrt(4.0) = 20.0 (greater than max_norm 15.0)
            FgMeasurement { res: 40.0, raw_var: 4.0, is_phase: true, sat: Some(sat2), h_row: DVector::zeros(0), weight: 1.0 },
            // Phase, norm = -60.0 / sqrt(9.0) = 20.0 (equal to current max_norm, shouldn't override because of >)
            FgMeasurement { res: -60.0, raw_var: 9.0, is_phase: true, sat: Some(sat1), h_row: DVector::zeros(0), weight: 1.0 },
        ];
        
        assert_eq!(PppFactorGraph::find_worst_outlier_sat(&meas), Some(sat2));

        let meas_no_outlier = vec![
            FgMeasurement { res: 10.0, raw_var: 4.0, is_phase: true, sat: Some(sat1), h_row: DVector::zeros(0), weight: 1.0 },
        ];
        assert_eq!(PppFactorGraph::find_worst_outlier_sat(&meas_no_outlier), None);
    }
