...
pub fn calibrate_intrinsics(config: &EngineConfig, obs: &[EpochObs]) -> (f64, f64) {
    ...
    for i in 1..obs.len() {
        ...
        for curr_sat in &curr_epoch.satellites {
            if let Some(prev_sat) = prev_epoch.satellites.iter().find(|s| s.sat == curr_sat.sat) {
                if let (Some(curr_pr), Some(prev_pr)) = (curr_sat.get_observable(1), prev_sat.get_observable(1)) {
    ...
    let mut drift_var = drift_estimates.iter().map(|d| {
        let diff = d - mean_drift;
        diff * diff
    }).sum::<f64>() / (drift_estimates.len() - 1) as f64;
...
mod tests {
    ...
    use gneiss_core::obs::{SatObs, Observation, ObsCode, ObsType, SignalCode};
        ...
        EpochObs {
            ...
            satellites: vec![
                SatObs {
                    ...
                    observations: vec![Observation { code, value: pr1, lock_time: None, lli: None }]
                },
                SatObs {
                    sat: SatelliteId { constellation: Constellation::Gps, prn: 2 },
                    observations: vec![Observation { code, value: pr2, lock_time: None, lli: None }]
                }
            ]
        }
    }
