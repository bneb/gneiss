...
impl ImuPreintegrationFactor {
    fn extract_all_states(&self, state: &DVector<f64>) -> (Vector3<f64>, Vector3<f64>, UnitQuaternion<f64>, Vector3<f64>, Vector3<f64>, Vector3<f64>, Vector3<f64>, UnitQuaternion<f64>, Vector3<f64>, Vector3<f64>) {
        ...
        (p_i, v_i, q_i, ba_i, bg_i, p_j, v_j, q_j, ba_j, bg_j)
    }
}

impl Factor for ImuPreintegrationFactor {
    fn residual(&self, state: &DVector<f64>) -> DVector<f64> {
        let (p_i, v_i, q_i, ba_i, bg_i, p_j, v_j, q_j, ba_j, bg_j) = self.extract_all_states(state);
        ...
        res.fixed_rows_mut::<3>(6)
            .copy_from(&(dq.inverse() * r_i_t * q_j).scaled_axis());
