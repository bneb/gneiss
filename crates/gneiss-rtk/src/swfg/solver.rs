...
/// Find active columns for a factor in the global variable values state vector.
fn find_active_columns(factor: &dyn Factor, values: &VariableValues, m_rows: usize, j: &DMatrix<f64>) -> Vec<usize> {
...
/// Accumulate a single factor's residual and Jacobian into the normal equations.
fn accumulate_factor_normal_equations(
    factor: &dyn Factor,
    values: &VariableValues,
    jtj: &mut DMatrix<f64>,
    jtr: &mut DVector<f64>,
...
/// Falls back to LU decomposition, and finally SVD if singular.
fn solve_linear_system(a: &DMatrix<f64>, b: &DVector<f64>) -> Option<DVector<f64>> {
    // Try Cholesky first (fast path for PSD matrices)
    if let Some(chol) = a.clone().cholesky() {
        return Some(chol.solve(b));
    }
    // Try QR decomposition (fast direct solve for general non-singular matrices)
    ...
    // Fall back to SVD pseudo-inverse as last resort
...
mod tests {
    ...
    fn solver_creates_epoch_variables() {
        ...
        // Pose(6) + Clock(3) + ZWD(1) = 3 variables when no IMU/GLONASS
    ...
    fn solver_lm_converges_with_prior_factor() {
        ...
        // Add prior factors for all created variables to satisfy graph connectivity
        for &id in &var_ids {
        ...
        // After LM, position should be near the prior
