...
impl DataSource for BkgProvider {
    ...
    async fn fetch_base_obs(
        ...
        tracing::info!("Saved Hatanaka compressed base data to {}", crx_file.display());
...
impl BkgProvider {
    pub async fn fetch_precise_products(&self, time: GpsTime, out_dir: &Path) -> Result<Vec<PathBuf>, FetchError> {
        let gps_week = time.week;
        let gps_dow = (time.tow / 86400.0).floor() as u32;

        let base_url = format!("https://igs.bkg.bund.de/root_ftp/IGS/products/mgex/{}/", gps_week);
        let files = [
        ...
        for f in &files {
            ...
            let mut dest = std::fs::File::create(&dest_z).map_err(|e| FetchError::Network(e.to_string()))?;
            let bytes = response.bytes().await?;
            ...
            let status = std::process::Command::new("gunzip")
                ...
                .arg(&dest_z)
                .status()
                .map_err(|e| FetchError::Decompression(format!("Failed to execute gunzip: {}", e)))?;

            if !status.success() {
                return Err(FetchError::Decompression(format!("gunzip failed with status {}", status)));
            }
