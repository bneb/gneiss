use gneiss_parsers::rtcm3::{parse_1019, parse_msm_message, parse_rtcm3_frame, RtcmParseError};
use std::fs;
...
fn test_real_world_rtcm3_parsing() {
        ...
        match parse_rtcm3_frame(&buffer) {
            Ok((remaining, frame)) => {
                ...
                if frame.payload.len() >= 2 {
                    let msg_num =
                        (u16::from_be_bytes([frame.payload[0], frame.payload[1]]) >> 4) & 0x0FFF;
    ...
    assert!(
        eph_frames > 0,
        "Expected to decode at least one ephemeris frame"
    );
