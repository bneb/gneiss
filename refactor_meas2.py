...
def main():
    ...
    for i, line in enumerate(lines):
        if line.startswith("#[allow(clippy::too_many_arguments)]") and "pub fn compute_innovations" in lines[i+1]:
            start_inn = i
        if start_inn != -1 and line.startswith("}") and i > 750:
            if "None" in lines[i-1] or "None" in lines[i-2]:
                end_meas = i
                break
...
"""
    final = "".join(lines[:start_inn]) + new_content + "".join(lines[end_meas+1:])
